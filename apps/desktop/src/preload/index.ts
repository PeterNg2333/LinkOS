...
const api = {
    llm: {
        ...
        chat: (messages: BaseMessageLike[], systemPrompt?: string, enableRAG?: boolean) => electronAPI.ipcRenderer.invoke("llm:chat", messages, systemPrompt, enableRAG),
        chatStream: (
            ...
            electronAPI.ipcRenderer.send("llm:chat-stream", traceId, messages, systemPrompt, enableRAG);
            return () => {
                removeChunkListener()
                removeErrorListener()
                removeEndListener()
